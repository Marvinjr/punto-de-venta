    ...
    <table cellspacing="0" cellpadding="0" border="0" id="flex1" class="table table-hovered">
        ...
        <tbody>
            <?php foreach ($list as $num_row => $row) : ?>
                ...
                $row                =   get_instance()->events->apply_filters( 'grocery_filter_row', $row, $rawList[ $num_row ] );
                    ...
                    <?php foreach ($columns as $column):?>
                        <td class="<?php echo (isset($order_by[0]) &&  $column->field_name == $order_by[0]) ? 'sorted' : null ?>">
                            <div style="width: 100%;" class='text-left'>
                                <?php echo xss_clean( $row->{$column->field_name});?>
                            </div>
                        </td>
                    <?php endforeach;?>
                        ...
                        <td align="left" width='10%'>
                            <div class="dropdown">
                                ...
                                if (!empty($row->action_urls)) {
                                    $data               =    get_instance()->events->apply_filters('grocery_filter_actions', [ $row->action_urls, $actions, $row, $rawList[ $num_row ] ]);
            ...
            if( confirm( '<?php echo _s( 'Would you like to delete selected entries', 'grocerycrud' );?>' ) ) {
                $.ajax({
                    ...
                    success     :   ( data ) => {
                        ...
                        $( this ).removeAttr( 'is-bound' );
                        $( '#ajax_refresh_and_loading' ).trigger( 'click' );
                    }
                });
            }   

            $( this ).attr( 'is-bound', 'yes' );     
        }
    });
});
</script>