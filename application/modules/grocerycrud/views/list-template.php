            ...
            <div class="row">
                <div class="col-lg-6 col-md-6 col-xs-12">
                    ...
                    ?></span> </div>
                        ...
                        <?php foreach( get_instance()->events->apply_filters( 'grocery_header_buttons', [] ) as $index => $button ):?>
                            <a class="btn btn-default" id="<?php echo @$button[ 'id' ] ?? ( 'button-' . $index );?>" href="<?php echo @$button[ 'url' ];?>">
...
if( layout.is( 'xs' ) || layout.is( 'sm' ) ) {
    $('.table-responsive').on('show.bs.dropdown', function () {
        $('.table-responsive').css( "overflow", "auto" );
        $( '#dropdown-for-mobile' ).remove();
        $( 'body' ).append( 
            `
            <div id="dropdown-for-mobile" style="
            height: 100%;
            width: 100%;
            position: fixed;
            top: 0;
            background: rgba(51, 51, 51, 0.21176470588235294);
            left: 0;
            z-index: 991;
            display: flex;
        ...
        setTimeout( () => {
            $( '.open > .dropdown-menu' ).siblings( 'a' ).eq(0).attr( 'restore-dropdown-menu', 'yes' );
            $( '.open > .dropdown-menu' ).appendTo( '#dropdown-for-mobile' );
            $( '#dropdown-for-mobile .dropdown-menu' ).css({
                'position': 'static',
                'display': 'inline-block',
                'width': '50%',
                'height': 'auto',
            });
            $( '#dropdown-for-mobile' ).bind( 'click', function( e ){
                e.stopPropagation();
                $( '#dropdown-for-mobile .dropdown-menu' ).css({
                    'position': 'absolute',
                    'display': 'none',
                ...
                $( '#dropdown-for-mobile .dropdown-menu' ).insertAfter( 'a[restore-dropdown-menu]' );
                $( 'a[restore-dropdown-menu' ).removeAttr( 'restore-dropdown-menu' );
                $( this ).remove();
