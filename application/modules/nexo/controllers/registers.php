...
class NexoRegistersController extends CI_Model
{
    public function crud_header()
    {
		...
		**/
		...
		if( ! $this->events->apply_filters( 'nexopos_show_registers', ! ( multistore_enabled() && ! is_multistore() ) ) ) {
			return show_error( __( 'Cette fonctionnalité est indisponible depuis l\'entrepôt.', 'nexo' ) );
		}
		...
		$crud->columns('NAME', 'USED_BY', 'NPS_URL', 'ASSIGNED_PRINTER', 'STATUS', 'TOTAL_CASH', 'AUTHOR', 'DATE_CREATION' );
        $crud->fields( $fields );
        ...
        $crud->display_as('TOTAL_CASH', __('Cash Disponible', 'nexo'));
         ...
         * @since 3.12.8
         */
        $crud->callback_column( 'TOTAL_CASH', function( $amount ) {
			return get_instance()->Nexo_Misc->cmoney_format( $amount );
		});
		
        $crud->callback_column( 'NPS_URL', function( $value ) {
			return empty( $value ) ? __( 'N/A', 'nexo' ) : $value;
		});
		
        $crud->callback_column( 'ASSIGNED_PRINTER', function( $value ) {
			return empty( $value ) ? __( 'N/A', 'nexo' ) : $value;
        });
		...
		if ( User::can( 'nexo.clear.registers-history' ) ) {
			$crud->add_action(
				__( 'Effacer l\'historique', 'nexo'),
				'',
				site_url(array( 'dashboard', store_slug(),  'nexo', 'clear-history' )) . '/',
				'clear_history fa fa-trash-o'
			);
		}
	...
	public function __use( $reg_id = 'default', $order_id = null )
	{
		...
		$this->events->add_action( 'dashboard_footer', function(){
			...
			**/
			...
			/**
			 * tax computing script
			 * @since 3.14.31
			 */
			get_instance()->load->module_view( 'nexo', 'checkout.v2-1.tax-compute-script' );
			get_instance()->load->module_view( 'nexo', 'checkout.v2-1.discount-script' );
			get_instance()->load->module_view( 'nexo', 'checkout.v2-1.categories-toggler' );
			get_instance()->load->module_view( 'nexo', 'checkout.v2-1.cash-drawer' );
			get_instance()->load->module_view( 'nexo', 'checkout.v2-1.cash-register' );
		});
		...
		if( ! in_array( @$Options[ $options_prefix . 'nexo_enable_registers' ], array( null, 'non' ) ) ){
			...
			$this->events->add_filter( 'checkout_header_menus_2', function( $menus ) {
				...
				$menu  		=	[
					...
					'text'  =>  __( 'Caisse Enregistreuse', 'nexo' ),
					'icon'  =>  'desktop',
					'type'  =>  'dropdown',
					'options'   =>  [
						[
				...
				/**
				 * If the use is allowed to 
				 * disburse cash registers
				 */
				if ( User::can( 'nexo.disbursing.registers' ) ) {
					$menu[ 'options' ][] 	=	[
						'class' =>  'default cash-out',
						'text'  =>  __( 'Décaissement', 'nexo' ),
						'icon'  =>  'money'
					];
				}
		...
		**/
		...
		/**
		 * we should make some prior verification before 
		 * opening the POS UI
		 */
		$this->events->do_action( 'pos_ready' );
		...
		$data[ 'orderedCategories' ] 	=	$this->category_model->getProductsCategoriesAscOrdered();
	...
	public function __register_grocery_filter_action( $data )
    {
        ...
        foreach ($actions as $key => $action) {
			...
			if( $url === site_url( array( 'dashboard', store_slug(), 'nexo', 'open', 'register' ) ) ) {
				if ( in_array( $rawList->STATUS, array( 'opened', 'locked' ) ) ){
			...
			if( $url === site_url( array( 'dashboard', store_slug(), 'nexo', 'register-history' ) ) ) {
				...
				if ( in_array( $rawList->STATUS, array( 'opened', 'locked' ) ) || ( User::cannot( 'nexo.view.registers-history' ) ) ){
					unset($grocery_actions_obj[ $key ]);
				}
			}

			if( $url === site_url( array( 'dashboard', store_slug(), 'nexo', 'use', 'register' ) ) ) {
				if ( in_array( $rawList->STATUS, array( 'closed',  'locked' ) ) ) {
					unset($grocery_actions_obj[ $key ]);
				}
			}

			if( $url === site_url( array( 'dashboard', store_slug(), 'nexo', 'close', 'register' ) ) ) {
				if ( in_array( $rawList->STATUS, array( 'locked', 'closed' ) ) ) {
					unset($grocery_actions_obj[ $key ]);
				}
			}

			if( $url === site_url( array( 'dashboard', store_slug(), 'nexo', 'clear-history' ) ) ) {
				if ( in_array( $rawList->STATUS, array( 'locked', 'opened' ) ) || ( User::cannot( 'nexo.view.registers-history' ) ) ) {
					unset($grocery_actions_obj[ $key ]);
				}
			}
		}

        return [ $grocery_actions_obj, $actions, $row, $rawList ];
    }
