...
class NexoCategoriesController extends CI_Model
{
    public function crud_header()
    {
		...
		**/
		...
		// @since 2.8		
        $fields					=	$this->events->apply_filters( 'nexo_categories_fields', [ 'NOM', 'PARENT_REF_ID', 'THUMB', 'DESCRIPTION' ]);
        $columns                =   $this->events->apply_filters( 'nexo_categories_columns', [ 'NOM',  'PARENT_REF_ID', 'DESCRIPTION', 'THUMB', 'DATE_CREATION' ]);
        ...
        $this->events->do_action( 'load_categories_crud', $crud );  
    ...
    public function reorder()
    {
        ...
        $categories     =   $this->category_model->getProductsCategoriesAscOrdered();
        
        $this->Gui->set_title( store_title( __( 'Réorganisation des catégories', 'nexo' ) ) );
        $this->load->module_view( 'nexo', 'categories.reorder-gui', compact( 'categories' ) );
    }

    public function catMerger()
    {
        $this->load->module_model( 'nexo', 'NexoCategories', 'category_model' );
        $categories     =   $this->category_model->getProductsCategoriesAscOrdered();
        $this->Gui->set_title( store_title( __( 'Fusion des catégories', 'nexo' ) ) );
        $this->load->module_view( 'nexo', 'categories.merger-gui', compact( 'categories' ) );
    }
}
