...
class NexoOrdersController extends CI_Model
{
    public function crud_header()
    {
		...
		**/
		...
		if( ! $this->events->apply_filters( 'nexopos_show_orders', ! ( multistore_enabled() && ! is_multistore() ) ) ) {
		...
		**/
        ...
        $crud->add_action(
			...
			'',
			$this->events->apply_filters( 'normal_print_receipt_url', site_url(array( 'dashboard', store_slug(), 'nexo', 'orders', 'receipt' )) ) . '/',
			'fa fa-file'
        );
        ...
        $crud->callback_column( 'TOTAL', function( $price ){
            ...
            return get_instance()->Nexo_Misc->cmoney_format( $price );
        });

        $crud->callback_column( 'TVA', function( $price ){
            get_instance()->load->model( 'Nexo_Misc' );
            return get_instance()->Nexo_Misc->cmoney_format( $price );
        });
    ...
    public function lists($page = 'home', $id = null)
    {
        ...
        $this->events->add_action( 'dashboard_footer', function() {
            ...
            get_instance()->events->do_action( 'nexo_orders_footer' );
        });
