...
class NexoProvidersController extends CI_Model
{
    public function crud_header()
    {
		...
		**/
		...
		// @since 2.8		
        $fields					=	array( 'NOM', 'BP', 'TEL', 'EMAIL', 'STREET', 'CITY', 'ZIPCODE', 'COUNTRY', 'FAX', 'WEBSITE', 'DESCRIPTION', 'DATE_CREATION', 'DATE_MOD' );
        ...
        $crud->display_as('STREET', __('Rue', 'nexo'));
        $crud->display_as('CITY', __('Ville', 'nexo'));
        $crud->display_as('ZIPCODE', __('Code Postal', 'nexo'));
        $crud->display_as('COUNTRY', __('Pays', 'nexo'));
        $crud->display_as('FAX', __('Fax', 'nexo'));
        $crud->display_as('WEBSITE', __('Site Web', 'nexo'));
    ...
    public function history( $provider_id, $page = 0 )
    {
        ...
        $config["base_url"] = dashboard_url([ 'providers_history', $provider_id ]);
