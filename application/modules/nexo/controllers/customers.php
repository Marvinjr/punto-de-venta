...
class NexoCustomersController extends CI_Model
{
    public function crud_header()
    {
		...
		**/
		...
		if( ! $this->events->apply_filters( 'nexopos_show_customers', ! ( multistore_enabled() && ! is_multistore() ) ) ) {
        ...
        $customer_columns   =   $this->events->apply_filters( 'nexo_clients_columns', [ 'NOM', 'EMAIL', 'OVERALL_COMMANDES', 'ALLOW_CREDIT', 'TOTAL_SPEND', 'REF_GROUP', 'AUTHOR', 'DATE_CREATION' ] );
        ...
        $crud->display_as( 'TOTAL_CREDIT', __( 'Solde Crédit', 'nexo' ) );
        $crud->display_as( 'ALLOW_CREDIT', __( 'Crédit Autorisé', 'nexo' ) );
        ...
        $crud->callback_column( 'ALLOW_CREDIT', function( $data ){
            switch( $data ) {
                case 'yes':
                    return __( 'Oui', 'nexo' );
                default:
        ...
        $crud->callback_column( 'TOTAL_CREDIT', function( $data ){
            get_instance()->load->model( 'Nexo_Misc' );
            return get_instance()->Nexo_Misc->cmoney_format( $data, true );
        });
        ...
        $crud->add_action( __( 'Rafraichir les dépenses', 'nexo' ), null, dashboard_url([ 'customers', 'refresh-expenditure' ] ) . '/', 'fa fa-refresh' );

        /**
         * adding option to the
         * customer account
         * @since 3.14.38
         */
        if ( store_option( 'enable_customers_accounts', 'no' ) === 'yes' ) {
            $crud->add_action( __( 'Créditer le compte', 'nexo' ), null, '', 'fa fa-money', function( $id, $row ) {
                return dashboard_url([ 'customers', 'account', $id, 'new' ] ) . '/';
            });
            $crud->add_action( __( 'Historique du compte', 'nexo' ), null, '', 'fa fa-bank', function( $id, $row ) {
                return dashboard_url([ 'customers', 'account', $id, 'history' ] ) . '/';
            });
        }
    ...
    **/
    ...
    public function groups_header( $page )
    {
		...
		**/
		...
		if( ! $this->events->apply_filters( 'nexopos_show_customers_groups', ! ( multistore_enabled() && ! is_multistore() ) ) ) {
			return show_error( __( 'Cette fonctionnalité a été désactivée.', 'nexo' ) );
		}
    ...
    **/
    ...
    public function resetCounter( $customer_id )
    {
        ...
        $this->reward_model->resetRewardCount( $customer_id );
    ...
    public function customerCouponsHeader( $customer_id )
    {
		...
		**/
		...
		if( ! $this->events->apply_filters( 'nexopos_show_coupons', ! ( multistore_enabled() && ! is_multistore() ) ) ) {
			return show_error( __( 'Cette fonctionnalité a été désactivée', 'nexo' ) );
		}
        ...
        $crud->display_as('USAGE_COUNT', __('Utilisation', 'nexo'));
        $crud->display_as('DISCOUNT_TYPE', __('Type de remise', 'nexo'));
    ...
    /**
     * render a vue to check the customers coupons
     * @param int customer id
     * @return view
     */
    ...
    public function accountHistory( $customer_id )
    {
        ...
        if ( $customer ) {
            $this->Gui->set_title( sprintf( __( 'Historique du compte du client : %s', 'nexo' ), $customer[ 'NOM' ] ) );
            return $this->load->module_view( 'nexo', 'customers.account.history-gui', compact( 'customer', 'customer_id' ), true );
        }

        return show_error( __( 'Impossible de trouver le client en utilisant l\'identifiant fourni', 'nexo' ) );
    }

    /**
     * Fill the customer account
     * @param int customer id
     * @return View
     */
    public function accountNew( $customer_id )
    {
        $this->load->module_model( 'nexo', 'NexoCustomersModel', 'customer_model' );
        $customer   =   $this->customer_model->getSingle( $customer_id );

        if( $customer ) {
            $this->Gui->set_title( sprintf( __( 'Créditer le compte du client : %s', 'nexo' ), $customer[ 'NOM' ] ) );
            return $this->load->module_view( 'nexo', 'customers.account.new-gui', compact( 'customer' ), true );
        }
    ...
    {
        $this->load->module_model( 'nexo', 'NexoCustomersModel', 'customer_model' );
        $result     =   $this->customer_model->refreshExpenditures( $customer_id );
        if ( $result[ 'status' ] === 'success' ) {
            return redirect( $_SERVER['HTTP_REFERER'] . '?notice=customer-data-refreshed' );
        }
        return redirect( $_SERVER['HTTP_REFERER'] . '?notice=an-error-occured' );
    }
}