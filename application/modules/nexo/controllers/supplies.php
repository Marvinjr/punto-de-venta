...
class NexoSuppliesController extends CI_Model
{
    ...
    public function __delete_supplies( $supply_id ) 
    {
        ...
        foreach( $supplies as $supply ) {
            ...
            if ( ! empty( $fresh_item ) ) {
                ...
                /**
                 * @todo : We might reduce supplier debt
                 */
            }
        }
    }
    ...
    **/
    ...
    public function delivery_invoice( $delivery_id )
    {
        ...
        $this->load->module_model( 'nexo', 'NexoStockTaking', 'procurement_model' );
        $procurement    =   $this->procurement_model->get( $delivery_id );

        if ( empty( $procurement ) ) {
            return show_404( __( 'Impossible de trouver l\'approvisionnement demandé.', 'nexo' ) );
        }
    ...
    **/
    ...
    public function delivery_items_crud( $delivery_id )
    {
		...
		**/
        ...
        $crud->callback_after_update([ $this, '__after_update_history' ]);
    ...
    **/
    ...
    public function __before_update_history( $post, $index )
    {
        ...
        if( $history ) {
            ...
            $result     =   $this->stocktaking_model->countItemsTotal( $history[0][ 'REF_SHIPPING' ], $index );            
            ...
            $post[ 'TOTAL_PRICE' ]          =   floatval( $post[ 'QUANTITE' ] ) * floatval( $post[ 'UNIT_PRICE' ]);
            ...
            $this->events->do_action_ref_array( 'update_supply_history', [ $post, $index, $history[0][ 'REF_SHIPPING' ] ]);
        }
        return $post;
    }

    public function __after_update_history( $post, $index )
    {
        $history        =   $this->db->where( 'ID', $index )
            ->get( store_prefix() . 'nexo_articles_stock_flow' )
            ->result_array();

        $this->events->do_action_ref_array( 'after_update_supply_history', [ $post, $index, $history[0][ 'REF_SHIPPING' ] ]);
    }
