...
if ( store_option( 'nexo_enable_registers' ) === 'oui' ):
    ...
    $this->Gui->add_item(array(
        'type'        =>    'select',
        'name'        =>    store_prefix() . 'nexo_allow_cash_out',
        'label'        =>    __( 'Autoriser la sortie d\'argent', 'nexo'),
        'description'   =>  __( 'Permet d\'autoriser une transaction sortante d\'argent de la caisse enregistreuse.', 'nexo' ),
        'options'    =>    array(
            ''		=>	__( 'Veuillez choisir une option', 'nexo' ),
            'yes'        =>    __('Oui', 'nexo'),
            'no'        =>    __('Non', 'nexo')
...
get_instance()->events->do_action( 'nexo_after_register_settings', [
    'gui'       =>  $this->Gui,
    'namespace' =>  $namespace
]);