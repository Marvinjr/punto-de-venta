	...
	<div style="background:#FFF;padding:15px;box-shadow:0 0 1px #333" class="invoice-container">
		...
		<div class="row invoice-info">
			...
			<div class="col-sm-4 col-xs-4">
				...
				<address>
					<p style="margin:0"><strong><?php echo __( 'Client :', 'nexo' );?></strong>
						{{ data.order[0].customer_name }}</p>
					<p style="margin:0"><strong><?php echo __( 'Téléphone', 'nexo' );?>:</strong> {{ billing.phone }}
					</p>
					<p style="margin:0"> <strong><?php echo __( 'Email', 'nexo' );?>:</strong>
						{{ billing.email || '<?php echo __( 'Non Disponible' );?>' }}</p>
					<p style="margin:0" ng-if="billing.address_1 != null && billing.address_1 != ''">
						<strong><?php echo __( 'Address 1 :', 'nexo' );?></strong>
						{{ billing.address_1 ? billing.address_1 + ',' : '' }}</p>
					<p style="margin:0" ng-if="billing.address_2 != null && billing.address_2 != ''">
						<strong><?php echo __( 'Address 2', 'nexo' );?></strong>
						{{ billing.address_2 ? billing.address_2 + ',' : '' }}</p>
					<p style="margin:0" ng-if="billing.city != null && billing.city != ''">
						<strong><?php echo __( 'Ville :', 'nexo' );?></strong>
						{{ billing.city ? billing.city + ',' : '' }}</p>
					<p style="margin:0" ng-if="billing.pobox != null && billing.pobox != ''">
						<strong><?php echo __( 'Boite Postale :', 'nexo' );?></strong>
						{{ billing.pobox ? billing.pobox + ',' : '' }}</p>
				</address>
			</div>
			...
			<div class="col-sm-4 col-xs-4">
				...
				<address>
					<p style="margin:0"><strong><?php echo __( 'Client :', 'nexo' );?></strong>
						{{ data.order[0].customer_name }}</p>
					<p style="margin:0"><strong><?php echo __( 'Téléphone', 'nexo' );?>:</strong> {{ shipping.phone }}
					</p>
					<p style="margin:0"><strong><?php echo __( 'Email', 'nexo' );?>:</strong> {{ shipping.email }}</p>
					<p style="margin:0" ng-if="shipping.address_1 != null && shipping.address_1 != ''">
						<strong><?php echo __( 'Address 1 :', 'nexo' );?></strong>
						{{ shipping.address_1 ? shipping.address_1 + ',' : '' }}</p>
					<p style="margin:0" ng-if="shipping.address_2 != null && shipping.address_2 != ''">
						<strong><?php echo __( 'Address 2', 'nexo' );?></strong>
						{{ shipping.address_2 ? shipping.address_2 + ',' : '' }}</p>
					<p style="margin:0" ng-if="shipping.city != null && shipping.city != ''">
						<strong><?php echo __( 'Ville :', 'nexo' );?></strong>
						{{ shipping.city ? shipping.city + ',' : '' }}</p>
					<p style="margin:0" ng-if="shipping.pobox != null && shipping.pobox != ''">
						<strong><?php echo __( 'Boite Postale :', 'nexo' );?></strong>
						{{ shipping.pobox ? shipping.pobox + ',' : '' }}</p>
				</address>
			</div>
			<!-- /.col -->
		</div>
		...
		<div class="row">
			...
			<div class="col-xs-6">
				<div class="table-responsive">
					<table class="table">
						...
						<th><?php echo sprintf( __( '%s (%s%%)', 'nexo' ), @$tax[0][ 'NAME' ], @$tax[0][ 'RATE' ] );?>
						</th>
						<td class="text-right">{{ data.order[0].TVA | moneyFormat }}</td>
						<?php endif;?>
						<?php endif;?>
						<?php endif;?>
						<tr ng-show="taxes" ng-repeat="(key, tax) in taxes">
							<th>{{ tax.NAME }}</th>
							<th class="text-right">{{ tax.VALUE | moneyFormat }}</th>
						</tr>
			...
			<div class="col-xs-6" ng-show="data.order[0].DESCRIPTION.length > 0">
				<?php echo sprintf( __( '<strong>Note :</strong> %s', 'nexo' ), '{{ data.order[0].DESCRIPTION }}' );?>
			</div>
			<!-- /.col -->
		</div>
		<div class="row">
			<div class="col-md-12">
				<div class="jumbotron" style="padding: 0px">
					<div class="container">
						<p class="text-center" style="margin: 1em; font-size: 12px"><?php echo store_option( 'nexo_bills_notices' );?></p>
					</div>
				</div>				
			</div>
		</div>

		<?php $this->events->do_action( 'nexo_invoice_footer' );?>
		<!-- /.row -->
