...
<document>
    ...
    <?php $this->events->do_action( 'nps_before_items', compact( 'items', 'order' ));?>
    <line-feed></line-feed>
    <text>
    ...
    <line-feed></line-feed>
    <?php $this->events->do_action( 'nps_before_details', compact( 'items', 'order', 'discount_amount', 'discountTotal' ) );?>
    <text>
        <text-line><?php echo nexting([], '*');?></text-line>   
    </text>
    <bold>
        ...
        <?php echo nexting([
            __( 'Sous Total', 'nexo' ),
            get_instance()->Nexo_Misc->cmoney_format( $order[ 'NET_TOTAL' ] )
        ]);?></text-line>
        <text-line><?php echo nexting([], '-');?></text-line>
        ...
        <?php elseif( store_option( 'nexo_vat_type' ) == 'item_vat' ):?>            
            ...
            <?php if ( $taxes ) {
                foreach( $taxes as $tax ) {
                    ?>
                    <text-line><?php echo nexting([
                        sprintf( __( '%s (%s%%)', 'nexo' ), $tax[ 'NAME' ], $tax[ 'RATE' ] ),
                        $this->Nexo_Misc->cmoney_format( $tax[ 'TOTAL_TAX' ] )
                    ]);?></text-line>
        ...
        <?php
        $view    =   ob_get_clean();
        echo $this->events->apply_filters( 'render_taxes_nps_basic_receipt', $view, compact(
            'order', 'discountTotal', 'subTotal'
        ));
        ?>
    ...
    <?php $this->events->do_action( 'nps_before_reimbursement', compact( 'order', 'items', 'discountTotal', 'totalRefunds', 'refunds', 'order_payments' ) );?>
    <align mode="center">
        <text-line size="2:2">
        <?php echo 
            $terme . ' ' .
    ...
    <line-feed></line-feed>
    <?php $this->events->do_action( 'nps_before_footer_note', compact( 'order' ) );?>
    <align mode="center">
        <text-line><?php echo xss_clean( $this->parser->parse_string( store_option( 'nexo_bills_notices' ), $template , true ) );?></text-line>
    </align>
    <?php $this->events->do_action( 'nps_after_footer_note', compact( 'order' ) );?>
		<?php 		
