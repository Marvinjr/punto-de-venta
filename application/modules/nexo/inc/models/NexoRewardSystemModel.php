    ...
    public function handleRewardIfEnabled( $data )
    {
        if ( store_option( 'nexo_enable_reward_system', 'no' ) === 'yes' ) {
            ...
            if ( $order[ 'TYPE' ] !== 'nexo_order_comptant' ) {
                log_message( 'info', sprintf( 'Impossible to handle the reward for the order %s, as it\'t not complete', $order[ 'CODE' ] ) );
                return false;
            }
            ...
            if ( $customer ) {
                ...
                log_message( 'info', sprintf( 'The customer %s doesn\'t seems to have a customer group. It\'s required to save the reward points on the order %s.', $customer[ 'NOM' ], $order[ 'CODE' ] ) );
                return [
                    'status'    =>  'failed',
            ...
            log_message( 'info', sprintf( 'Impossble to find the customer who should benefit the reward on the order %s', $order[ 'CODE' ] ) );
            return [
                'status'    =>  'failed',
        ...
        log_message( 'info', 'Unable to proceed with the reward system. It\'s disabled.' );
        return [
            'status'    =>  'failed',
