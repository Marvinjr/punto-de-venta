...
trait Nexo_items
{
    ...
    **/
    ...
    {
        $prefix     =   'store-' . get_store_id() . '-';

        /**
         * create the cache dir so that it could be deleted
         */
        if ( ! is_dir( APPPATH . '/cache/pos' ) ) {
            mkdir( APPPATH . '/cache/pos' );
        }
        ...
        if ( $id != null && ! in_array( $filter, [ 'sku-barcode', 'search' ] ) ) {
			...
			' . store_prefix() . 'nexo_articles.ORDER as ORDER,
			' . store_prefix() . 'nexo_categories.ID as CAT_ID
			')
            ->from( store_prefix() . 'nexo_articles')
            ->order_by( store_prefix() . 'nexo_articles.ORDER', 'asc' )
        ...
        $this->response( 
            $this->events->apply_filters_ref_array( 'nexopos_pos_retreived_item', [ $result, $filter ]), 
            $result ? 202 : 404 
        );
    }
    ...
    **/
    ...
    {
        $this->load->module_model( 'nexo', 'NexoStockTaking', 'stock_model' );
        $provider       =   $this->post( 'ref_provider' );
        $procurement    =   $this->post( 'selected_delivery' );

        $response   =   $this->stock_model->save_supply( 
            $this->post( 'items' ),
            $provider[ 'ID' ],
            $procurement[ 'ID' ],
            $this->post( 'new_delivery' )
        );
