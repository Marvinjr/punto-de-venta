...
trait Nexo_orders
{
    ...
    **/
    ...
    public function order_post($author_id)
    {
        try {
            ...
            return $this->response( $this->events->apply_filters( 'post_order_response', array(
    ...
    **/
    ...
    public function order_put($author_id, $order_id)
    {
        ...
        $order_details            =    array(
            ...
            'NET_TOTAL'             =>      $this->put( 'NET_TOTAL' ),
            'AUTHOR'                =>      $author_id,
            'PAYMENT_TYPE'          =>      $this->put('PAYMENT_TYPE') ?? 'unpaid',
        ...
        $order_details          =   $this->events->apply_filters( 'put_order_details', $order_details, $this->put(), $order_id, $current_order );

        $this->db->where( 'ID', $order_id )
        ->update( store_prefix() . 'nexo_commandes', $order_details);

        /**
         * let's release old quantity
         * to avoid computing that twice
         */
        $this->order_model->releaseOrderProducts( $old_order[ 'order' ][0] );
		...
		if( is_array( $this->put( 'payments' ) ) ) {
			foreach( $this->put( 'payments' ) as $payment ) {
                ...
                $paymentResponse    =   $this->order_model->addPayment(
                    $current_order[0][ 'ID' ],
                ...
                // make sure the coupon has not yet been saved as a payment.
                if( $payment[ 'namespace' ] == 'coupon' && ! isset( $payment[ 'meta' ][ 'saved' ] ) ) {
        ...
        if ( $current_order[0][ 'TYPE' ] === 'nexo_order_comptant' ) {
            ...
            if ( $this->put('REF_CLIENT') != $old_order['order'][0][ 'REF_CLIENT' ] ) {
                ...
                tendoo_debug([
                    ...
                    'old_order'     =>  $old_order['order' ][0][ 'TOTAL' ],
                    'new_total'     =>  $this->put( 'TOTAL' )
                ], 'order-coupon-debug.json' );
        ...
        **/
        foreach ( $this->put('ITEMS') as $item ) {
            ...
            $this->events->do_action_ref_array( 'after_order_product_created', [
                'id'            =>  $command_product_id,
                'item_data'     =>  $item_data,
                'order'         =>  $current_order,
                'item'          =>  $item,
            ]);
            ...
            if ( $current_order[0][ 'TYPE' ] === 'nexo_order_comptant' ) {
                ...
                **/
                if( intval( $item['stock_enabled'] ) == 1 && ! ( bool ) intval( $item[ 'inline' ] ) && intval( $fresh_item[0][ 'TYPE' ] ) !== 3 ) {
                    $item_stock_data                       =   [];
                ...
                $this->item_model->holdQuantity( $fresh_item[0][ 'ID' ], $item[ 'qte_added' ] );
            }
            ...
            $this->events->do_action( 'after_order_item_saved', [
                'product_id'    =>  $command_product_id,
                'data'          =>  $item_data,
                'order'         =>  $current_order,
                'item'          =>  $item,
                'freshItem'     =>  $fresh_item
            ]);
        }
    ...
    **/
    ...
    public function order_by_dates_post($order_type = 'all', $register = null )
    {
        ...
        $this->db       =   $this->events->apply_filters( 'nexopos_daily_report_query', $this->db );
    ...
    */
    ...
    public function order_with_item_post( $order_id = null )
    {
        ...
        $this->db       =   $this->events->apply_filters( 'nexopos_orders_with_items_query', $this->db );

        $result     =   $this->db
        ->get()->result();

        foreach( $result as &$_result ) {
            $_result->category   =   $this->category_model->getSingle( $_result->REF_CATEGORIE );
        }
	...
	**/
	...
	public function order_payment_post( $order_id )
	{
        ...
        $somme_percu        =   round( $somme_percu, store_option( 'decimal_precision', 0 ) );
    ...
    **/
    ...
    public function sales_detailed_post()
    {
        ...
        $query  =   $this->events->apply_filters( 'nexopos_sales_detailed_query', $query );
