    ...
    public function install_tables( $scope = 'default', $prefix = '' )
    {
		...
		**/
        ...
        $this->db->query('CREATE TABLE IF NOT EXISTS `'.$table_prefix.'nexo_clients` (
            ...
            `ALLOW_CREDIT` varchar(200) NOT NULL,
        ...
        $this->db->query('CREATE TABLE IF NOT EXISTS `'.$table_prefix.'nexo_clients_accounts` (
            `ID` int(11) NOT NULL AUTO_INCREMENT,
		...
		**/
        ...
        $this->db->query('CREATE TABLE IF NOT EXISTS `'.$table_prefix.'nexo_articles` (
            ...
            `PRIX_DE_VENTE_BRUT` FLOAT NOT NULL,
		...
		**/
		...
		$short_prefix 	=	$prefix;
		$this->events->do_action_ref_array( 'nexo_after_install_tables', array( $table_prefix, $scope, $short_prefix ) );
  }
	...
	public function uninstall($namespace, $scope = 'default', $prefix = '')
	{
		...
		if ($namespace === 'nexo') {
			...
			foreach( $stores as $store ) {
				...
				// var_dump( $table_prefix . $store_prefix );die;

				$this->events->do_action_ref_array( 'nexo_before_delete_tables', array( $table_prefix . $store_prefix, $scope, $store ) );
				...
				$this->db->query('DROP TABLE IF EXISTS `'.$table_prefix. $store_prefix . 'nexo_commandes_refunds`;');
				$this->db->query('DROP TABLE IF EXISTS `'.$table_prefix. $store_prefix . 'nexo_commandes_refunds_products`;');
				// @since 3.0.16
				...
				$this->db->query('DROP TABLE IF EXISTS `'.$table_prefix. $store_prefix . 'nexo_daily_log`;');
				$this->db->query('DROP TABLE IF EXISTS `'.$table_prefix. $store_prefix . 'nexo_rewards_rules`;');
				$this->db->query('DROP TABLE IF EXISTS `'.$table_prefix. $store_prefix . 'nexo_rewards_system`;');
				$this->db->query('DROP TABLE IF EXISTS `'.$table_prefix. $store_prefix . 'nexo_taxes`;');
				$this->db->query('DROP TABLE IF EXISTS `'.$table_prefix. $store_prefix . 'nexo_rayons`;');
				$this->db->query('DROP TABLE IF EXISTS `'.$table_prefix. $store_prefix . 'nexo_notices`;');
				...
				$this->db->query('DROP TABLE IF EXISTS `'.$table_prefix. $store_prefix . 'nexo_clients_accounts`;');
	...
	**/
	...
	public function create_permissions()
	{
		...
		foreach([ 
			...
			foreach([ 'create.', 'edit.', 'delete.', 'view.', 'clear.', 'disbursing.' ] as $action ) {

				$permission 	=	'nexo.' . $action . $component;

				if ( in_array( $permission, $permissions_keys ) ) {

					$this->aauth->allow_group( 'store.manager', $permission );
					$this->aauth->allow_group( 'master', $permission );
