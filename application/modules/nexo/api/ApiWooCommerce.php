    ...
    public function syncDownSingleOrder()
    {
        ...
        try {
            ...
            $orderData      =   $this->events->apply_filters( 'woocommerce_post_order_data', [
                'order'     =>  [
                    'shipping'              =>  [
                        'id'                =>  $customerCreationResponse[ 'customer' ][ 'shipping' ][ 'id' ] ?? 0,
            ...
            file_put_contents( 'sync-down-order-response.txt', json_encode( $orderData[ 'order' ] ) );
    
            return $this->response( $response );
        } catch( Exception $e ) {
            file_put_contents( 'sync-down-order-failure.txt', $e->getMessage() );
        }
    }
