...
class NexoAdvancedStoreManagerApp extends CI_Model {
    ...
    public function __construct()
    {
        ...
        $this->events->add_filter( 'np_registers_history_users', [ $this, 'filter_register_report_users' ], 10, 1 );
    }

    /**
     * filter the users 
     * that apper on the register
     * history report
     * @param array users
     * @return array users filtred
     */
    public function filter_register_report_users( $users )
    {
        if ( multistore_enabled() && is_multistore() ) {
            $users  =   array_filter( $users, function( $user ) {
                if ( get_option( 'store_access_' . $user->user_id . '_' . get_store_id() ) === 'yes' ) {
                    return true;
                }
                return false;
            });
    ...
    public function before_init_store( $details )
    {
        if ( ! User::in_group( 'master' ) && get_option( 'store_access_' . User::id() . '_' . $details[ 'store_id' ] ) !== 'yes' ) {
            return show_error( __( 'You\'re not allowed to access to that store. Contact the store manager for more informations.', 'nsam' ) );
        }
    }
    ...
    **/
    ...
    public function load_dashboard()
    {
        ...
        if( ! multistore_enabled() && User::can( 'toggle_modules' ) ) {
            nexo_notices([
                'user_id'   =>  User::id(),
        ...
        if( is_multistore() && User::in_group([ 'store.cashier', 'store.manager', 'store.demo' ]  ) ) {
    ...
    **/
    ...
    {
        if( multistore_enabled() && is_multistore() && User::in_group([ 'master', 'store.manager' ] ) ) {
