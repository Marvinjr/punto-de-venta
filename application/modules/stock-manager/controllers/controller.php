...
class Nexo_Stock_Manager_Controller extends Tendoo_Module
{
    ...
    /**
     * Crud Header
     * @todo must be checked as the id generated by grocerycrud for each row is now dynamic
     * @return void
    **/
    ...
    public function history_crud()
    {
		...
		**/
        ...
        // XSS Cleaner
        $this->events->add_filter( 'grocery_filter_row', function( $row, $rawRow ) {

            if( $rawRow->APPROUVED_BY === '0' ) {
                $row->sa2ab2bc9      =   __( 'N/A', 'stock-manager' );
            }

            if( $rawRow->FROM_STORE === '0' ) {
                $row->sa3130f62 =   __( 'Main Warehouse', 'stock-manager' );
            }

            if( $rawRow->DESTINATION_STORE == '0' ) {
        ...
        $this->events->add_filter( 'grocery_filter_actions', function( $data ) {
            ...
            $row            =   $data[2];
            $rawRow         =   $data[3];
            $query          =   $this->transfert_model->get( $rawRow->ID );

            if( $query[0][ 'STATUS' ] === 'pending' && intval( $rawRow->DESTINATION_STORE ) == get_store_id() ) { // means pending
            ...
            } else if( $query[0][ 'STATUS' ] === 'pending' && intval( $rawRow->FROM_STORE ) == get_store_id() ) {
            ...
            } else if( $query[0][ 'STATUS' ] === 'requested' && intval( $rawRow->FROM_STORE ) == get_store_id() ) {
            ...
            return [ $urls, $actions, $row, $rawRow ];
        }, 10 );
