...
class Transfert_model extends Tendoo_Module
{
    ...
    **/
    ...
    public function approve_transfert( $data )
    {
        ...
        $this->load->module_model( 'nexo', 'NexoCategories' );
        ...
        $source_items_details           =   nexo_convert_raw( $raw_items, 'SKU', null, false );
        $source_categories_details      =   nexo_convert_raw( $raw_categories, 'NOM' );
        $current_categories_details     =   nexo_convert_raw( $raw_current_categories, 'NOM' );

        foreach( $source_categories_details as &$__cat ) {
            unset( $__cat[ 'ID' ] );
        }
        foreach( $current_categories_details as &$__cat ) {
            unset( $__cat[ 'ID' ] );
        }

        // var_dump(  );die;
        // echo json_encode( $source_categories_details );die;
        ...
        foreach( $items as $item ) {
            ...
            if( @$storeItems[ $item[ 'SKU' ] ] === null ) {
                ...
                if( @$source_items_details[ $item[ 'SKU' ] ] != null ) {
                    ...
                    $data   =   $this->createCategoryForItem( compact( 
                        'item_data', 
                        'item', 
                        'source_items_details', 
                        'current_categories_details',
                        'source_categories_details', 
                        'errors',
                        'slug',
                        'store_prefix'
                    ) );
                ...
                if ( $transfert[0][ 'STATUS' ] === 'requested' ) {
                    ...
                    $itemOnCurrentStore     =   @$storeItems[ $item[ 'SKU' ] ];
                }

                $item_data          =   @$storeItems[ $item[ 'SKU' ] ];
            }

            if( ! empty( $item_data ) ) {
                /**
                 * This means that the deduction hasn't been
                 * made when the process (request, transfer)
                 * has been send
                 */
                if ( $transfert[0][ 'DEDUCT_FROM_SOURCE' ] === 'no' ) {
                     ...
                     */
                    $canTransfert       =   ( floatval( @$source_items_details[ $item[ 'SKU' ] ][ 'QUANTITE_RESTANTE' ] ) - floatval( $item[ 'QUANTITY' ] ) ) > 0;
                ...
                $this->db->where( 'SKU', $item[ 'SKU' ] )->update( $store_prefix . 'nexo_articles', [
                ...
                if( @$transfert[0][ 'DEDUCT_FROM_SOURCE' ] == 'no' ) {
                    $item_details           =   $this->db->where( 'SKU', $item[ 'SKU' ] )
                ...
                $this->events->do_action( 'nexo_approve_item_stock_transfer', [
                    'item'      =>  $itemOnCurrentStore,
                    'quantity'  =>  $item[ 'QUANTITY' ],
                    'from'      =>  $transfert[0][ 'FROM_STORE' ],
                    'to'        =>  $transfert[0][ 'DESTINATION_STORE' ]
                ]);
            }
        }
    ...
    private function createCategoryForItem( $data )
    {
         ...
         */
        $category   =   $this->db
            ->where( 'ID', $item_data[ 'REF_CATEGORIE' ] )
            ->get( $slug . 'nexo_categories' )
            ->result_array();

        if ( empty( $category ) ) {
            $errors[]       =   [
                'status'    =>  'failed',
                'message'   =>  __( 'Unable to find the requested category using the id ' . $item_data[ 'REF_CATEGORIE' ] )
            ];
        } else {
            if( @$current_categories_details[ slugify( $category[0][ 'NOM' ] ) ] == null ) {
                if( @$source_categories_details[ slugify( $category[0][ 'NOM' ] ) ] != null  ) {  
                    /**
                     * checks if the category hasn't yet been created
                     */
                    $localCategory   =   $this->db->where( 'NOM', $category[0][ 'NOM' ] )
                        ->get( $store_prefix . 'nexo_categories' )
                        ->result_array();
                    
                    $category_fields    =   list_table_fields( $store_prefix . 'nexo_categories' );
                    
                    if ( empty( $localCateogry ) ) {
                        $this->db->insert( 
                            $store_prefix . 'nexo_categories', 
                            collect( $source_categories_details[ slugify( $category[0][ 'NOM' ] ) ])->filter( function( $value, $key ) use ( $category_fields ) {
                                return in_array( $key, $category_fields );
                            })->toArray()
                        );

                        $item_data[ 'REF_CATEGORIE' ]   =  $this->db->insert_id();
                    } else {
                        $item_data[ 'REF_CATEGORIE' ]   =   $localCategory[0][ 'ID' ];
                    }
                }
            }
        }
    ...
    public function reject_transfert( $data )
    {
        ...
        foreach( $items as $item ) {
            ...
            if( ! $itemExists ) {
                $this->db->where( 'SKU', $item[ 'SKU' ] )->update( $from_prefix . 'nexo_articles', [
                    'QUANTITE_RESTANTE'             => floatval( $item_details[0][ 'QUANTITE_RESTANTE' ] ) + floatval( $item[ 'QUANTITY' ] )
                ]);
            }                
        }
    ...
    public function transfert( $data )
    {
        ...
        $this->db->insert( 'nexo_stock_transfert', $transfert_data );
        ...
        foreach( $items as  $item ) {
            ...
            $this->events->do_action( 'nexo_send_item_stock_transfer', [
                'transfer_id'   =>  $transfert_id,
                'item'          =>  $singleItem,
                'quantity'      =>  $item[ 'QTE_ADDED' ],
                'is_request'    =>  $data[ 'is_request' ],
                'from'          =>  $transfert_data[ 'FROM_STORE' ],
                'to'            =>  $transfert_data[ 'DESTINATION_STORE' ],
                'type'          =>  'transfert_out'
            ]);
        }   
    ...
    public function verify( $data )
    {
        ...
        if( $transfert[0][ 'STATUS' ] === 'requested' ) {
            ...
            $exhaustedItems     =   [];
            foreach( $items as $item ) {
                $fullItem   =   $this->Nexo_Products->get_single( $item[ 'SKU' ], 'SKU' );
                if( ! empty( $fullItem ) ) {
                    /**
