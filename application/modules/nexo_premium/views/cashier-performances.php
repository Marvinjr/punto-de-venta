...
$this->events->do_action( 'hook_into_cashier_performances_dom' );
...
var NexoCashierPerformance	=	new function(){
	...
	**/
	...
	this.GetStats			=	function( cashier_id, start_date, end_date ){
		...
		const fetchURL 		=	NexoAPI.events.applyFilters( 'cashier_performance_url', '<?php echo site_url(array( 'api', 'nexopos', 'cashiers_performance' ));?>' + '<?php echo store_get_param( '?' );?>' );
		$.ajax( fetchURL, {
			data	:	{
				cashier_id,
				start_date,
