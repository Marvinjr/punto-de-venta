...
$this->Gui->add_meta(array(
	 ...
	 *
	**/
	$this->events->do_action( 'hook_cash_flow_dom' );
	?>
	...
	$( document ).ready(function(e) {
		$( '.circonstrire' ).bind( 'submit', function() {
			if( $( '#report_date' ).val() != '' ) {
				document.location =	'<?php echo site_url(array( 'dashboard', store_slug(), 'nexo', 'reports', 'cash-flow' ));
		?>/' + $( '#report_date' ).val();
			} else {
		...
		**/
		...
		this.GetReport	=	function() {
			if( this.Index < this.EntryLength ) {
				...
				const getURL 	=	NexoAPI.events.applyFilters( 'cash_flow_url', '<?php echo site_url(array( 'nexo_premium', 'flux', store_get_param( '?' ) ));?>' );
				$.post(
					getURL,
					data,
					function( content ) {
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel"><?php _e('Chargement en cours...', 'nexo_premium');?></h4>
				</div>
