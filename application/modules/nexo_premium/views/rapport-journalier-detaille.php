...
<?php $this->events->do_action( 'hook_daily_report_dom' );?>
		...
		**/
		...
		?>
			...
			this.OrderReport = function () {
				const orderByDateURL 	=	NexoAPI.events.applyFilters( 'order_by_date_url', `<?php echo site_url(array( 'rest', 'nexo', 'order_by_dates' ));?>/all/` + this.Register_ID + '<?php echo store_get_param( '?' );?>' );
				$.post(
					orderByDateURL,
					_.object(
						['start', 'end'],
