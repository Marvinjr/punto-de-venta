...
*
**/
$this->events->do_action( 'hook_sales_stats_dom' );
?>
	...
	**/
	...
	this.__FillTableValue			=	function( depth ) {
		if( this.Index < this.EntryLength ) {
			// Looping IDS
			const fetchURL 	=	NexoAPI.events.applyFilters( 'annual_sales_url', '<?php echo site_url(array( 'nexo_premium', 'sales_stats' ));?>' + '/' + this.CurrentDate + '<?php echo store_get_param( '?' );?>' );
			$.ajax( fetchURL, {
				beforeSend	:	function(){
