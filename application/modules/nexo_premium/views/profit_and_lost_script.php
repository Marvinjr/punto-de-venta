...
$table_head             =   $this->events->apply_filters( 'np_profit_lost_report_thead_row', array(
    ...
    'category'      =>  array(
        'text'          =>  _s( 'Categorie', 'nexo_premium' ),
        'class'         =>  '',
        'width'         =>  300
    ),
    'sku'            =>  array(
...
$table_column           =   $this->events->apply_filters( 'np_profit_lost_report_tbody_row', array(
    ...
    'category'            =>  array(
        'text'          =>  '{{ item.category.NOM }}',
        'class'         =>  '',
        'attr'          =>  '',
        'csv_field'       =>  'item.category.NOM'
    ),
    'sku'            =>  array(
        'text'          =>  '{{ item.SKU }}',
        'class'         =>  '',
    ...
    */
    ...
    $scope.getSales     =   function(){
        if( ! angular.isUndefined( $scope.startDate ) &&  ! angular.isUndefined( $scope.endDate ) || true ) {
            const salesURL      =   NexoAPI.events.applyFilters( 'profit_lost_url', '<?php echo site_url( array( 'rest', 'nexo', 'order_with_item' ) );?>' + '?<?php echo store_get_param( null );?>' );
            $http.post( salesURL, {
