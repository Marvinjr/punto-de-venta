...
class NexoPremiumReportController extends Tendoo_Api
{
                ...
                foreach ($Dates as $date_key => &$content) {
                    if (is_array($cashier_id)) {
                        foreach ($cashier_id as $id) {
                            ...
                            $this->db       =   $this->events->apply_filters( 'nexopos_cashiers_performance_query', $this->db );
    ...
    public function stockTracking()
    {
        ...
        $this->db       =   $this->events->apply_filters( 'nexopos_stock_tracking_query', $this->db );

        $deliveries     =   $this->db->get()
        ->result_array();
