...
class NexoPremiumController extends Tendoo_Module
{
    ...
    **/
     ...
     * @return void
     */
    public function dailyProductTaxReport()
    {
        if ( User::cannot( 'nexo.read.today-report' ) ) {
            return nexo_access_denied();
        }

        $this->events->add_action( 'dashboard_footer', function() {
            get_instance()->load->module_view( 'nexo_premium', 'product-taxes.script' );
        });
        
        $this->load->module_model( 'nexo', 'NexoTaxesModel', 'taxes_model' );

        $taxes      =   $this->taxes_model->get();

        $this->Gui->set_title( store_title( __( 'Rapport sur les taxes collectées', 'nexo_premium' ) ) );
        $this->load->module_view( 'nexo_premium', 'product-taxes/gui', compact( 'taxes' ) );
    }
}