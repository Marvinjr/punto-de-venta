...
class Nexo_Checkout extends CI_Model
{
    ...
    **/
    ...
    public function shuffle_code()
    {
        ...
        if ( store_option( 'nexo_code_type', 'order_code' ) == 'date_code' ) {
            ...
            $index          =   get_option( 'nexo_orders_indexes_' . store_prefix() . slugify( $todayString ), 0 );
            $code           =   substr( $date->year, 2 ) . sprintf("%02d", $date->month ) . sprintf("%02d", $date->day ) . '-' . sprintf( "%03d", $index );
            $this->order_model->updateOrderCode();
            return $code;
        } else {
            $length         =   6;
    ...
    **/
    ...
    public function commandes_delete($post, $force = false )
    {
        $this->load->module_model( 'nexo', 'Nexo_Orders_Model', 'order_model' );

        if ( class_exists( 'User' ) && $force === false ) {
            // Protecting
        ...
        if ( $command[0][ 'TYPE' ] === 'nexo_order_comptant' ) {
             ...
             */
            $this->order_model->releaseOrderProducts( $command[0] );
        }
    ...
    **/
    ...
    /**
     * count all available order
     * made during a specific time range
     * @param string date
     * @param string date
     * @return array
     */
    public function count_all_orders( $start = null, $end = null )
    {
        $query      =   $this->db->from( store_prefix() . 'nexo_commandes' );

        if( ! empty( $start ) && ! empty( $end ) ) {
            $query->where( 'DATE_CREATION >=', $start )
    ...
    **/
    ...
    public function postOrder( $data, $author_id )
    {
        ...
        $order_details              =       array(
            ...
            'CODE'                  =>      $this->shuffle_code(),
            'RABAIS'                =>      $data[ 'RABAIS' ],
            'GROUP_DISCOUNT'        =>      $data[ 'GROUP_DISCOUNT' ],
            'TOTAL'                 =>      $data[ 'TOTAL'],
            'NET_TOTAL'             =>      $data[ 'NET_TOTAL' ],
            'AUTHOR'                =>      $author_id,
            'PAYMENT_TYPE'          =>      $data[ 'PAYMENT_TYPE' ] ?? 'unpaid',
		...
		if( is_array( @$data[ 'payments' ] ) ) {
            foreach( $data[ 'payments' ] as $payment ) {
                ...
                // tendoo_debug( $paymentResponse, 'order-payments-' . $current_order[0][ 'CODE' ] . '.json');
                ...
                if( $payment[ 'namespace' ] == 'coupon' ) {
                    ...
                    $this->db->insert( store_prefix() . 'nexo_commandes_coupons', [
                        ...
                        'REF_PAYMENT'   =>  $paymentResponse[ 'data' ][ 'payment_id' ]
                    ]);
                }

                if ( $payment[ 'namespace' ] === 'account' ) {
                    $this->load->module_model( 'nexo', 'NexoCustomersModel', 'customer_model' );
                    $this->customer_model->creditAction( 
                        $current_order[0][ 'REF_CLIENT' ],
                        'payment',
                        floatval( $payment[ 'amount' ] ),
                        __( 'Paiement effectuée sur une commande', 'nexo' )
                    );
                }
            }
        }
        ...
        **/
         ...
         * in order to make to allow additionnal
         * offers
         */
    ...
    public function afterOrderPlaced( $post_data )
    {
        ...
        **/
        ...
        $updated_details    =   $this->events->apply_filters( 'after_order_placed_details', $updated_details, $data, $fresh_order );
