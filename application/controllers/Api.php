	...
	*/
	public function index( $page_slug ) 
	{
		...
		$Routes->group([ 'prefix' => substr( request()->getHeader( 'script-name' ), 0, -10 ) . '/api' ], function() use ( $page_slug, $Routes ) {
			...
			$modules        =   Modules::get( null, 'mu-modules' );
			foreach( $modules as $namespace => $module ) {
				if( is_dir( $dir = MU_MODULESPATH . $namespace . '/api/' ) ) {
					foreach( glob( $dir . "*.php") as $filename) {
						include_once( $filename );
					}
				}

				if( is_file( MU_MODULESPATH . $namespace . '/api.php' ) ) {
					include_once( MU_MODULESPATH . $namespace . '/api.php' );
				}
			}
