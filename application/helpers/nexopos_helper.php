...
function nexo_convert_raw( $raw, $key, $value = null, $slugify = true ) {
    $new    =   [];
    foreach( $raw as $data ) {
        if ( $slugify ) {
            $new[ slugify( $data[ $key ] ) ]      =   ( $value == null ) ? $data : $data[ $value ];
        } else {
 ...
 * @return array
 */
function nexo_log( $title, $description )
{
    get_instance()->load->module_model( 'nexo', 'NexoLogModel', 'log_model' );
    return get_instance()->log_model->log( $title, $description );
}
...
function stores()
{
    get_instance()->load->module_model( 'nexo', 'Nexo_Stores_Model', 'stores_model' );
    $stores     =   get_instance()->stores_model->getAllStores();
    return collect( $stores )->map( function( $store ) {
        $store[ 'TABLE_PREFIX' ]    =   get_instance()->db->dbprefix( 'store_' . $store[ 'ID' ] . '_' );
        return $store;
    });
}