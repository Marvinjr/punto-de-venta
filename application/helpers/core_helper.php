...
function pagination( $config )
{
    ...
    /**
     * @param string $table
     * @param array $where collection of where statement
     * @param string $search_on COLUMN where search should be made
     */
    extract( $config );

    $perPage            =   floatval( get_instance()->input->get( 'per_page' ) ?? @$perPage ?? 10 );
    $totalEntries       =   get_instance()->db->count_all( store_prefix() . $table );
    $totalPages         =   ceil( $totalEntries / $perPage );
    $query              =   get_instance()->db;

    /**
     * this will make the query hookable
     */
    get_instance()->events->do_action( 'paginating_' . $table, $query, $config );

    $query->limit( $perPage, $perPage * ( $page - 1 ) )
        ->order_by( $orderBy ?? 'ID', $orderByDirection ?? 'desc' );

    if ( ! empty( get_instance()->input->get( 'search' ) ) && @$search_on ) {
        if ( is_array( $search_on ) ) {
            foreach( $search_on as $column ) {
                $query->or_like( $column, get_instance()->input->get( 'search' ) );
            }
        } else {
            $query->like( $search_on, get_instance()->input->get( 'search' ) );
        }
    }
    ...
    $entries            =   $query->get( store_prefix() . $table )
    ->result();
...
/**
 * List the actual database fields
 * @param string table to fetch from
 * @return array list of available fields
 */
function list_table_fields( $table )
{
    if ( get_instance()->db->table_exists( $table ) ) {
        $table_name     =   get_instance()->db->dbprefix . get_instance()->db->escape_str( $table );
        $data           =   get_instance()->db->query( 'SHOW COLUMNS FROM `' . $table_name . '`' )->result_array();
